...
name: schliff:init
description: >
  Initialize Schliff tracking for a skill. Auto-generates an eval suite
  ...
  /schliff to start the improvement loop.
---

# /schliff:init

Initialize Schliff improvement tracking for a skill.
...
9. Present the initialization summary:
   ...
   ✓ Results log: schliff-results.jsonl
   ✓ History: schliff-history/exp-000-baseline.md

   Ready to grind. Run /schliff to start the loop.
   ```
...
- The schliff-history directory preserves snapshots of the SKILL.md at each iteration, allowing you to compare versions and revert if needed.
