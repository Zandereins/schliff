...
description: >
  ...
  failures. Reads from .skillforge/failures.jsonl.
---
...
1. Locate the failure log:
   ```bash
   cat .skillforge/failures.jsonl 2>/dev/null | head -5
   ```
   If empty or missing: "No failures logged yet. Use `/skillforge:log-failure` to record issues, or failures are auto-logged during eval runs."
...
- Failures are logged by `/skillforge:log-failure` or automatically during eval runs
- The session hook (`session-injector.js`) surfaces untriaged failures at session start
- After fixes are applied, re-run `/skillforge:eval` to verify improvement
