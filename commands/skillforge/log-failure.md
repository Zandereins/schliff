---
name: skillforge:log-failure
description: >
  Log a skill failure for later triage. Records the failure with context
  to .skillforge/failures.jsonl for clustering and analysis by /skillforge:triage.
---

# /skillforge:log-failure
...
4. Append to the failure log:
   ```bash
   mkdir -p .skillforge
   echo '<JSON_ENTRY>' >> .skillforge/failures.jsonl
   ```

5. If the file exceeds 1 MB, warn the user:
   ```
   Warning: failures.jsonl is getting large. Consider running /skillforge:triage
...
6. Confirm the log entry:
   ...
   Run /skillforge:triage to cluster and prioritize all logged failures.
   ```

## Notes

- Failures are stored in `.skillforge/failures.jsonl` (project-scoped)
- The session hook automatically surfaces 3+ untriaged failures at session start
- Run `/skillforge:triage` to cluster failures and generate fixes
