...
>
> Schliff is a deterministic quality scorer purpose-built for AI agent instruction files like CLAUDE.md and SKILL.md. It scores files across 8 dimensions (clarity, structure, constraints, examples, edge cases, context, specificity, coherence), runs as a pre-commit hook or GitHub Action, and provides an autonomous improvement loop that rewrites files to hit a target score. Zero dependencies, Python 3.9+ stdlib only, 732 tests, MIT license.
>
...
>
> Schliff scores CLAUDE.md, SKILL.md, and other AI agent instruction files across 8 quality dimensions. It integrates as a pre-commit hook or GitHub Action, catching quality regressions before they ship. The autonomous improvement loop rewrites files to reach a target score without manual editing. Pure Python stdlib, zero dependencies, 732 tests.
>
...
- **PR Title:** Add Schliff — 8-dimension quality scorer for skill files
- **PR Body:**

> Add [Schliff](https://github.com/Zandereins/schliff) to the Development & Quality section.
>
> Schliff is a deterministic scorer that evaluates SKILL.md and CLAUDE.md files across 8 dimensions: clarity, structure, constraints, examples, edge cases, context, specificity, and coherence. It includes an autonomous improvement loop that iteratively rewrites skill files to hit a configurable target score. Ships with a GitHub Action and pre-commit hook for CI integration.
>
...
- **Category:** Multi-Language (or new category: "AI Instruction Files")
- **PR Title:** Add Schliff — static analysis for AI agent instruction files
- **PR Body:**

> Add [Schliff](https://github.com/Zandereins/schliff) under a new "AI Instruction Files" category (or Multi-Language if preferred).
>
> Schliff performs deterministic static analysis on AI agent instruction files (CLAUDE.md, .cursorrules, AGENTS.md, SKILL.md). It evaluates 8 quality dimensions and produces a 0-100 composite score. Unlike LLM-based linters, scoring is fully deterministic and reproducible. Zero external dependencies, Python 3.9+ stdlib only, MIT license, 732 tests.
>
...
## 5. caramelomartins/awesome-linters

- **Repo:** https://github.com/caramelomartins/awesome-linters
- **Category:** AI/LLM or Markdown
- **PR Title:** Add Schliff — linter and quality scorer for AI instruction files
- **PR Body:**

> Add [Schliff](https://github.com/Zandereins/schliff) to the AI/LLM section (or Markdown if no AI category exists yet).
>
> Schliff lints and scores AI agent instruction files (CLAUDE.md, .cursorrules, AGENTS.md) across 8 quality dimensions. It flags missing sections, vague constraints, absent examples, and structural issues. Integrates via pre-commit hook and GitHub Action. Deterministic scoring with zero dependencies, Python 3.9+ stdlib only.
>
...
## 6. vintasoftware/python-linters-and-code-analysis
...
>
> Schliff is a pure-Python linter and quality scorer for AI agent instruction files. It uses only the Python 3.9+ standard library with zero external dependencies. Scores files across 8 dimensions (clarity, structure, constraints, examples, edge cases, context, specificity, coherence) and includes an autonomous rewrite loop. 732 tests, MIT license, available on PyPI (`pip install schliff`).
>
> A good example of a focused, stdlib-only Python tool that addresses a new file category emerging in modern development workflows.

---

## 7. ml-tooling/best-of-python-dev
...
>
> Schliff is a deterministic quality scorer for AI agent instruction files (CLAUDE.md, .cursorrules, AGENTS.md, SKILL.md). It evaluates 8 dimensions, integrates as a GitHub Action and pre-commit hook, and includes an autonomous improvement loop. Pure Python 3.9+ stdlib, zero dependencies, 732 tests, MIT license.
>
...
## 8. rohitg00/awesome-claude-code-toolkit
...
>
> Schliff scores CLAUDE.md, SKILL.md, and other AI agent instruction files across 8 quality dimensions with fully deterministic, reproducible results. It ships with a GitHub Action, pre-commit hook, and an autonomous improvement loop (`schliff improve`) that rewrites files to reach a target score. Zero dependencies, 732 tests, MIT license.
>
...
>
> Schliff provides a Model Context Protocol server that exposes deterministic quality scoring for AI agent instruction files (CLAUDE.md, .cursorrules, AGENTS.md). Agents can request scores across 8 dimensions and trigger autonomous improvement loops via MCP tool calls. Pure Python, zero dependencies, MIT license.
>
> **Note:** Submit this PR once the MCP server implementation is complete and documented. Enables any MCP-compatible agent to lint and improve its own configuration files programmatically.

---

## 10. sourcegraph/awesome-code-ai
...
>
> Schliff establishes a measurable quality standard for AI agent instruction files — the CLAUDE.md, .cursorrules, AGENTS.md, and similar files that increasingly ship with codebases. It scores files across 8 dimensions, integrates into CI via GitHub Action and pre-commit hook, and includes an autonomous improvement loop. Deterministic, zero dependencies, Python 3.9+ stdlib only, 732 tests.
>
