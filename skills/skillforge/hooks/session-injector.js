...
function readFailures(failuresPath) {
  ...
  let stat;
  try { stat = fs.statSync(failuresPath); } catch { return []; }
  ...
  if (skipped > 0) {
    process.stderr.write(`[skillforge] session-injector: skipped ${skipped} malformed JSONL lines\n`);
  }
  return entries;
}
...
function markInjected(failuresPath, entries) {
  ...
  try {
    ...
    process.stderr.write(`[skillforge] session-injector: failed to mark injected: ${err.message}\n`);
...
function main() {
  ...
  process.stdin.on('end', () => {
    ...
    try {
      ...
      if (!path.isAbsolute(resolvedCwd) || !fs.existsSync(resolvedCwd) || !fs.statSync(resolvedCwd).isDirectory()) {
        process.stdout.write('{}');
        return;
      }

      const failuresPath = path.join(resolvedCwd, '.skillforge', 'failures.jsonl');
      ...
      if (untriaged.length >= MIN_UNTRIAGED) {
        ...
        for (const entry of untriaged) {
          ...
          if (clusters[key].examples.length < 2) {
            clusters[key].examples.push(sanitize(entry.assertion_id || entry.description || 'no details'));
          }
        }
      ...
      process.stderr.write(`[skillforge] session-injector error: ${err.message}\n`);
    }
