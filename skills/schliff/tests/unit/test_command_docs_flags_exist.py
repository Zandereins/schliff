...
# `bash scripts/<name>.sh` … up to the end of the fenced block
_INVOCATION = re.compile(
    r"bash\s+scripts/(?P<script>[\w.-]+\.sh)(?P<args>(?:[^\n`]|\\\n)*(?:\n\s+[^\n`]*)*)"
)
...
# A `case` branch in the argument parser: `        --flag)` or `        --a|--b)`.
_CASE_BRANCH = re.compile(r"^[ \t]+(--[\w-]+(?:\|--[\w-]+)*)\)", re.MULTILINE)


def _flags_accepted_by(script: str) -> set[str]:
    """Flags the script's argument parser actually branches on.
