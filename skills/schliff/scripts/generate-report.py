...
def _build_badge_markdown(score: float, grade: str) -> str:
    """Build a shields.io badge markdown string for a Schliff score."""
...
def resolve_baseline(progress: dict[str, Any]) -> tuple[Optional[float], dict[str, float]]:
    """Resolve the baseline composite and per-dimension scores.

    Falls back to current_best when no explicit baseline entry exists, so the
    markdown and JSON paths compute identical baseline/delta math.
    """
    ...
    return baseline_composite, baseline_scores
...
def format_report(skill_name: str, progress: dict[str, Any], current: dict[str, Any]) -> str:
    ...
    if "error" in progress and not progress.get("total_experiments"):
        lines.append(f"# Schliff Report: {skill_name}")
        lines.append("")
        lines.append("> No improvement data yet. Run `/schliff:auto` first.")
        lines.append("")
        lines.append(f"---\n*Generated by [Schliff](https://github.com/Zandereins/schliff) | {timestamp}*")
        return "\n".join(lines)

    # Handle scorer failure (timeout/crash/JSON-decode) explicitly rather than
    # treating the placeholder composite 0.0 as a real score.
    if current.get("error"):
        lines.append(f"# Schliff Report: {skill_name}")
        lines.append("")
        lines.append(f"> Scoring failed: {current['error']}")
        lines.append("")
        lines.append(f"---\n*Generated by [Schliff](https://github.com/Zandereins/schliff) | {timestamp}*")
        return "\n".join(lines)
    ...
    baseline_composite, baseline_scores = resolve_baseline(progress)
    ...
    lines.append(f"> Just ran Schliff on my Claude Code skill: {before_str} \u2192 {after_str}"
...
def build_json_output(
    ...
    # Surface scorer failure (timeout/crash/JSON-decode) instead of reporting
    # the placeholder composite 0.0 as a real score.
    if current.get("error"):
        return {
    ...
    baseline_composite, _ = resolve_baseline(progress)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate a shareable Schliff markdown report"
    )
    parser.add_argument("results_path", help="Path to schliff-results.jsonl")
