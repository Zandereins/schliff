#!/bin/bash
##############################################################################
# Schliff Integration Test Suite
#
  ...
  "test_cases": [
    {
      ...
      "prompt": "test",
      "assertions": [
        {"type": "contains", "value": "SCHLIFF", "description": "Case insensitive match"}
      ]
    }
  ],
  "edge_cases": []
  ...
  "test_cases": [
    {
      ...
      "assertions": [
        ...
        {"type": "excludes", "value": "Schliff", "description": "Excludes existing text"}
      ]
    }
  ],
  "edge_cases": []
...
mkdir -p "$_INJECT_DIR/.schliff"
printf '{"skill":"test\u200B\u200Cskill","failure_type":"test","injected":false}\n' \
    > "$_INJECT_DIR/.schliff/failures.jsonl"
...
if echo "$_INIT_RESULT" | python3 -c "import sys,json; d=json.load(sys.stdin); assert d['skill_name']=='schliff'; assert d['triggers']['positive']>=5" 2>/dev/null; then
...
if echo "$_REPORT" | grep -q "# Schliff Report"; then
    pass "generate-report.py: markdown starts with '# Schliff Report'"
else
...
if echo "$AI_TEXT" | grep -q "Schliff Auto-Improve Complete"; then
...
if 'Test Title' in banner and 'Schliff' in banner:
    passed += 1
else:
